...
Runnable today with MockTracker; swaps to GridTrackNet when TRK-1 is filled.
"""
...
def run_clip_pair(
    ...
    # 1. Sync (clap): fractional frame offset applied to camera B
    off = frame_offset(video_a, video_b, fps=fps, cfg=cfg.get("sync", {}))
    ...
    # 3. Track the ball in each view (§5.5)
    if tracker_a is None or tracker_b is None:
        t = GridTrackNetTracker(cfg.get("paths.tracker_weights"),
                                cfg.get("paths.gridtracknet_repo"),
                                cfg.get("tracking.min_confidence", 0.3))
