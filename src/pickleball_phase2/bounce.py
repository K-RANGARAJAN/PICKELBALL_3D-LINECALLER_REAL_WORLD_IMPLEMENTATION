...
Placeholder: BNC-1 (learned bounce-vs-paddle classifier — needs real rallies).
"""
...
def classify_bounce_vs_hit(track: BallTrack, frame: float) -> float:
    """PLACEHOLDER[BNC-1]: probability that the event at `frame` is a surface
    bounce rather than a paddle/body hit (§5.7 corroborating cues: proximity
    to ground vs player, horizontal-velocity preservation, ballistic-fit
    residual, optional acoustics). Needs real rally data — Recipe 6.
    Until implemented, the pipeline treats separation-confirmed events as
    bounces (safe on your staged ground-truth clips, NOT on free rallies).
    """
    raise NotImplementedError("PLACEHOLDER[BNC-1] — see PLACEHOLDER_COOKBOOK.md, Recipe 6")
